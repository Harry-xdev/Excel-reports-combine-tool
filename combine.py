...
def running():
	...
	print('Deleted all old converted files.')

	file_format = ['*.xlsx', '*.xls']
	count = []
	for i in file_format:
		i = func_get_filename.get_filenames('input', i)
		count = count + i
		print(count)
	if count == []:
		print('Error! Please input files want to combine!')
	else:
		print('Tool starting...')
		...
		if df_list == []:
			print('Error! df_list is empty!')
		else:
			print('.')
			print('.')
			print('.')
			print('.')
			print('Amount of file: ', len(count))
			combined_df = combine_df_to_a_DataFrame(df_list)
			write_combined_df_to_excel(export_file_link, combined_df)
